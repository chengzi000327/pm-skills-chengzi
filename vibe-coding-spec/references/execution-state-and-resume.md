...
2. 只读当前 phase 需要的 artifact（implement 阶段读 `tasks.md` + `plan.md`；clarify 阶段读 `spec.md` + `clarify.md`），不要全量重读。
...
- 2026-06-11 (001-feature-name): 引入 SQLite 持久化，schema 见 specs/001-feature-name/data-model.md
...
- 用 `gh issue create` 批量创建；issue 编号回写到 `tasks.md` 对应 task 行和 `traceability.md`。
