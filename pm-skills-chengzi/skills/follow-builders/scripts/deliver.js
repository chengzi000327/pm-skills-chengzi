...
async function sendLark(text, delivery) {
  const larkCliBin = process.env.LARK_CLI_BIN || 'lark-cli';
  const args = ['im', '+messages-send'];
  const identity = delivery.as || 'user';

  args.push('--as', identity);

  if (delivery.profile) {
    args.push('--profile', delivery.profile);
  }
  ...
  if (delivery.chatId) {
    args.push('--chat-id', delivery.chatId);
  } else if (delivery.userId) {
    args.push('--user-id', delivery.userId);
  } else {
    throw new Error('delivery.chatId or delivery.userId is required for lark delivery');
  }
...
async function main() {
  ...
  try {
    switch (delivery.method) {
      ...
      case 'lark':
      case 'lark-cli': {
        const output = await sendLark(digestText, delivery);
        console.log(JSON.stringify({
          status: 'ok',
          method: 'lark-cli',
          message: delivery.dryRun ? 'Digest prepared for Feishu/Lark dry run' : 'Digest sent to Feishu/Lark',
